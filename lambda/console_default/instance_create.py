...
def create_action(region, name, server_name, secret_id, version, open_jdk_ver, instance_type, max_user, script_arg, update_plugins):
    ...
    def get_script(secret_id, arg_value, version, server_name, max_user, open_jdk_ver, update_plugins):
        user_data = ""
        with open('res/bash/user_data.sh') as f:
            user_data = f.read()
        return user_data.replace('%%SECRET_ID%%', secret_id).replace('%%ON_MOUNT_ARG%%', '%s %s %s %s %s %s' % (arg_value, version, server_name, max_user, open_jdk_ver, update_plugins))
    ...
    resp = ec2.run_instances(
        ...
        UserData=to_base64(get_script(secret_id, script_arg, version, server_name, max_user, open_jdk_ver, update_plugins)),
        NetworkInterfaces=[
            {
