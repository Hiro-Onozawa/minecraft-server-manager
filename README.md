...
デフォルトでVPCが作成されているはずなので、それがあればよい。
無ければとりあえずパブリックサブネットを1つ以上持ったVPCを作成する。

実装上はVPCを列挙した中の最初のVPCを選択している。

### Security Group

`MinecraftDefaultSecurityGroup` の名前でSecurityGroupを作成しておく。
...
`MinecraftDefaultKeyPair` の名前でKey Pairを作成しておく。
...
* ロール名: `Minecraft_Server_Instance_Role`
...
`default-minecraft-archive-bucket` の名前でバケットを作成しておく。
